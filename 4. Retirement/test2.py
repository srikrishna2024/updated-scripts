...
# Basic Info tab should come BEFORE investment details
tab1, tab2, tab3 = st.tabs(["Basic Info", "Investment Details", "Retirement Planning"])

# Tab 1: Basic Info - Define retirement_age and other basic variables first
with tab1:
...
# Tab 3: Retirement Planning - Define retirement age
with tab3:
    col1, col2 = st.columns(2)
    with col1:
...
# Tab 2: Investment Details - Now we can use retirement_age
with tab2:
    ...
    available_investment = max(0, current_salary - current_expenses * (1 + inflation_rate))
    st.write(f"Available amount for investment: {format_indian_number(available_investment)}")
    
    col1, col2 = st.columns(2)
    with col1:
        equity_percentage = st.slider(
            'Equity Allocation (%)',
            min_value=0,
            max_value=100,
            value=60,
            help="Percentage of investment allocated to equity"
        )
        annual_equity_investment = available_investment * equity_percentage/100
        st.write(f"Annual Equity Investment: {format_indian_number(annual_equity_investment)}")
    
    with col2:
        debt_percentage = 100 - equity_percentage
        st.write(f"Debt Allocation: {debt_percentage}%")
        annual_debt_investment = available_investment * debt_percentage/100
        st.write(f"Annual Debt Investment: {format_indian_number(annual_debt_investment)}")
    
    # Timeline selector
    selected_year = st.slider(
        "Years Before Retirement to Analyze",
        min_value=0,
        max_value=max(0, retirement_age - current_age),
        value=0,
        help="0 means retirement year, maximum value is years until retirement"
    )
    
    selected_age = retirement_age - selected_year
    st.write(f"Analyzing allocation for age {selected_age} ({selected_year} years before retirement)")
...
if st.button('Calculate Projections'):
    ...
    # Update the plotting section to include three plots:
    st.subheader('Portfolio Projections')
    
    # Plot 1: Income, Expenses and Investments
    # Plot 1: Income, Expenses and Investments
    fig1 = go.Figure()
    
    fig1.add_trace(go.Scatter(x=base_df['Age'], 
                             y=base_df['Salary'] + base_df['Other Income'],
                             name='Total Income', 
    ...
    fig1.add_trace(go.Scatter(x=base_df['Age'], 
                             ...
                             line=dict(color='red')))
    fig1.add_trace(go.Scatter(x=base_df['Age'], 
                             y=base_df['Equity Balance'],
                             name='Equity Balance', 
                             line=dict(color='blue')))
    fig1.add_trace(go.Scatter(x=base_df['Age'], 
                             y=base_df['Debt Balance'],
                             name='Debt Balance', 
                             line=dict(color='orange')))
    
    fig1.add_vline(x=selected_age, 
                   line_dash="dash", 
                   line_color="gray",
                   annotation_text=f"Selected Age: {selected_age}")
    ...
    st.subheader('Portfolio Projection with Uncertainty')
    fig2 = go.Figure()
    ...
    # Plot 3: Retirement Sufficiency
    st.subheader('Retirement Corpus Sufficiency')
    fig3 = go.Figure()
    ...
    required_corpus = retirement_expenses * 25
    
    fig3.add_trace(go.Scatter(x=base_df.loc[base_df['Age'] >= retirement_age, 'Age'], 
                             y=required_corpus,
                             name='Required Corpus (25x)', 
                             line=dict(color='red', dash='dash')))
    fig3.add_trace(go.Scatter(x=base_df['Age'], 
                             y=base_df['Total Portfolio'],
                             name='Projected Portfolio', 
                             line=dict(color='blue')))
    
    fig3.add_vline(x=selected_age, 
                   line_dash="dash", 
                   line_color="gray",
                   annotation_text=f"Selected Age: {selected_age}")
